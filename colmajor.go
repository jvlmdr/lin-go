...
func (A ContiguousColMajor) Size() Size {
	return Size{A.Rows, A.Cols}
}

func (A ContiguousColMajor) At(i, j int) float64 {
	return A.Elements[j*A.Rows+i]
}

func (A ContiguousColMajor) Set(i, j int, x float64) {
	A.Elements[j*A.Rows+i] = x
}

func (A ContiguousColMajor) RowMajor() bool {
	return false
}

func (A ContiguousColMajor) Array() []float64 {
	return A.Elements
}
...
func (A ContiguousColMajor) T() ContiguousRowMajor {
	return ContiguousRowMajor{A}
}
...
//
// The returned matrix references the same data.
func (A ContiguousColMajor) Resize(s Size) ContiguousColMajor {
...
func (A ContiguousColMajor) Submatrix(r Rect) SubContiguous {
	// Extract from first to last element.
	a := r.Min.I + r.Min.J*A.Rows
	b := (r.Max.I - 1) + (r.Max.J-1)*A.Rows + 1
	return ContiguousColMajorSubmatrix{r.Rows, r.Cols, A.Rows, A.Elements[a:b]}
}
